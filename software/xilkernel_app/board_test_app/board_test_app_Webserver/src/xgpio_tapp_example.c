...
int hello_led2 (int ledpos)
{
	int Status;
	Xuint16 DeviceId = GPIO_OUTPUT_DEVICE_ID;

	Status = XGpio_Initialize(&GpioOutput, DeviceId);	// is it needed ???
	...
	XGpio_DiscreteWrite(&GpioOutput, 1, 1 << ledpos);

	sleep(500);

	XGpio_DiscreteClear(&GpioOutput, 1, 1 << ledpos);
...
int hello_switch2 (char* log)
{
    ...
    // Switch 1
    strcat(log,  "    Switch 1: ");
    if(InputData & 0x1) strcat(log, "ON\n");
    else                strcat(log, "OFF\n");

    // Switch 2
    strcat(log,  "    Switch 2: ");
    if(InputData & 0x2) strcat(log, "ON\n");
    else                strcat(log, "OFF\n");

    // Switch 3
    strcat(log,  "    Switch 3: ");
    if(InputData & 0x4) strcat(log, "ON\n");
    else                strcat(log, "OFF\n");

    // Switch 4
    strcat(log,  "    Switch 4: ");
    if(InputData & 0x8) strcat(log, "ON\n");
    else                strcat(log, "OFF\n");
