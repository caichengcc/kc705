...
/**
*
* @file SwConfig_no_heap.c
*
* @brief This file contains software configuration implementation without
*        heap memory, which is practical in environments with low memory. It
*        is used for redirection of common system operations, namely:
*        - timers: redirected to plfTimer
*        - memory: simple heap only for static allocations (no freeing)
*        - thread locks: empty implementation
...
#define SHEAP_SIZE          10240
#define SHEAP_MARK_LENGTH   0
#define SHEAP_USED_MASK     (0x1<<31)


/************************* Variable Definitions *****************************/

static unsigned char SimpleHeap[SHEAP_SIZE] = {0};
static unsigned long SimpleHeapPos = 0;
...
void *swConfig_malloc(OsSizeT size)
{
    ...
    size += SHEAP_MARK_LENGTH;
    if (size > SHEAP_SIZE - SimpleHeapPos)
    {
        ERROR_PRINT("Not enough heap (%ld > %ld)\n\r", (unsigned long)size, (unsigned long)(SHEAP_SIZE - SimpleHeapPos));
        return NULL;
    }

#if SHEAP_MARK_LENGTH > 0
    (OsU32)&SimpleHeap[SimpleHeapPos] = SHEAP_USED_MASK | size;
#endif

    retPtr = &SimpleHeap[SimpleHeapPos + SHEAP_MARK_LENGTH];
...
{
    ERROR_PRINT("Freeing memory is not supported.\n\r");
}
